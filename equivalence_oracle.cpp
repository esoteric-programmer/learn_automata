...
Automaton* EquivalenceOracle::get_automaton() {
	return this->c->get_automaton();
}

EquivalenceOracle::EquivalenceOracle(Concept* c, std::vector<std::string> counter_examples) {
	if (!c) {
...
Alphabet EquivalenceOracle::get_alphabet() {
	return this->c->get_alphabet();
}

bool EquivalenceOracle::decide(Word word) {
	this->mq++;
	return this->c->decide(word);
}

Word EquivalenceOracle::difference(Concept* other, int algorithm) {
	return this->difference(other, algorithm, true);
}

Word EquivalenceOracle::difference(Concept* other, int algorithm, bool count_query) {
	if (count_query) {
		this->eq++;
	}
	size_t i;
...
void EquivalenceOracle::reset_queries() {
	this->eq = 0;
	this->mq = 0;
}
unsigned long int EquivalenceOracle::get_eq_queries() {
	return this->eq;
}
unsigned long int EquivalenceOracle::get_mq_queries() {
	return this->mq;
}

EquivalenceOracle::~EquivalenceOracle() {
}
